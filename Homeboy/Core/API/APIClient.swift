...
actor APIClient {
    ...
    private var currentProjectConfig: ProjectConfiguration {
        ConfigurationManager.readCurrentProject()
    }
    
    private var currentProjectId: String {
        currentProjectConfig.id
    }
    
    private var baseURL: String {
        currentProjectConfig.api.baseURL
    }
    
    var isAuthConfigured: Bool {
        let config = currentProjectConfig.api
    ...
    func initialize(onAuthFailure: @escaping () -> Void) {
        ...
        let tokens = KeychainService.getTokens(for: currentProjectId)
    ...
    func logout() async {
        if accessToken != nil {
            ...
            _ = try? await post("/auth/logout", body: request, requiresAuth: true) as EmptyResponse
        }
    ...
    private func clearAuth() {
        ...
        refreshToken = nil
        accessExpiresAt = nil
        KeychainService.clearTokens(for: currentProjectId)
    }
    
    // MARK: - Project Switching
    
    /// Clears cached tokens and reloads from Keychain for the new active project.
    ...
    func resetForProjectSwitch() {
        ...
        // Reload tokens for the new project
        guard isAuthConfigured else { return }
        let tokens = KeychainService.getTokens(for: currentProjectId)
        self.accessToken = tokens.accessToken
        self.refreshToken = tokens.refreshToken
